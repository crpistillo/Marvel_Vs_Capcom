...
void TCPServer::updateModel() {
	...
	while (this->clientsConnected != 0) {
			...
			//cout<<"clientsConnected: "<<clientsConnected<<endl;
				...
				int distancia[2];
				distancia[0] = computeDistance(team[0]->get_currentCharacter(),
						team[1]->get_currentCharacter());
				distancia[1] = computeDistance(team[1]->get_currentCharacter(),
						team[0]->get_currentCharacter());
